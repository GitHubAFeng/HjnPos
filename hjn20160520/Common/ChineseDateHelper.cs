...
namespace hjn20160520.Common
{
    ...
    public class ChineseDateHelper
    {
        ...
        ///<summary>
        /// 根据公历获取农历日期,返回整数数组，0下标为月，1下标为日
        ///</summary>
        ///<param name="datetime">公历日期</param>
        ///<return s></return s>
        public static int[] GetChineseDateTimeInt(DateTime datetime)
        {
            int[] date = new int[2];
            int year = ChineseCalendar.GetYear(datetime);
            int month = ChineseCalendar.GetMonth(datetime);
            ...
            //月 
            date[0] = isleap ? -1 : GetMonthInt(month);  //如果为负-1则没有二月
            date[1] = day;
        ...
        ///<param name="month">月份</param>
        ///<return s></return s>
        public static int GetMonthInt(int month)
        {
            int temp = 0;
            if (month < 13 && month > 0)
            {
